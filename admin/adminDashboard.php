...
// Fetch students in the selected group with grade info
$studentsQuery = "
    SELECT u.UsersID, u.Name,
           (SELECT AVG(sub.Grade) FROM Submissions sub WHERE sub.UsersID = u.UsersID AND sub.Grade IS NOT NULL) as AvgGrade,
           (SELECT COUNT(*) FROM Assignments asn 
            JOIN Student_Groups sg2 ON sg2.GroupID = asn.GroupID 
            WHERE sg2.UsersID = u.UsersID 
              AND asn.AssignmentID NOT IN (
                  SELECT s.AssignmentID FROM Submissions s WHERE s.UsersID = u.UsersID AND s.Status IN ('Submitted','Graded')
              )
           ) as MissingWork
    FROM Users u
            ...
            <ul class="period-list">
                <?php foreach ($groups as $g): ?>
                    <li class="period-item <?php echo ((int)$g['GroupID'] === $selectedGroup) ? 'active' : ''; ?>">
                        <a href="adminDashboard.php?group=<?php echo $g['GroupID']; ?>">
                ...
                <table class="student-table">
                    ...
                    <tbody>
                        <?php while ($student = $studentsResult->fetch_assoc()):
                            ...
                            $attClass = strtolower($todayAtt);

                            // Grade display
                            list($gradeText, $gradeClass) = getLetterGrade($avgGrade);

                            // Missing work
                            $missing = (int)$student['MissingWork'];
                            $missingText = $missing > 0 ? $missing . ' Assignment' . ($missing > 1 ? 's' : '') : 'None';
                            $missingColor = $missing > 0 ? 'red' : 'inherit';
                        ?>
                        <tr>
                            <td data-label="Student Name">
                                <strong><?php echo htmlspecialchars($student['Name']); ?></strong><br>
                                <span style="font-size: 0.8rem; color: #888;">ID: <?php echo $student['UsersID']; ?></span>
                            </td>
                            <td data-label="Status"><span style="color: <?php echo $statusColor; ?>;"><?php echo $status; ?></span></td>
                            ...
                            <td data-label="Actions"><a href="edit.php?UsersID=<?php echo $student['UsersID']; ?>">Edit</a></td>
                        </tr>
                        <?php endwhile; ?>
                        <?php if ($totalStudents === 0): ?>
                        <tr><td colspan="6" style="text-align:center;padding:1rem;">No students in this group.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </section>
